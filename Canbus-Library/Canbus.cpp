					...
					if (mcp2515_get_message(&message)) 
					{
							if((message.id == PID_REPLY) && (message.data[2] == pid))	// Check message is the reply and its the right PID
							{
								...
								{   /* Details from http://en.wikipedia.org/wiki/OBD-II_PIDs */
									...
									case MAP:				// Manifold Absolute Pressure, A, kPa (absolute)
										engine_data = message.data[3];
										sprintf(buffer,"%d",(int) engine_data);
									break;
							
									case BAROMETRIC:				// Barometric Pressure, A, kPa (absolute)
										engine_data = message.data[3];
										sprintf(buffer,"%d",(int) engine_data);
									break;
							
									case AMBIENT_TEMP: 	// 	A-40			  [degree C]
										engine_data =  message.data[3] - 40;
										sprintf(buffer,"%d",(int) engine_data);						
									break;

									case AF_RATIO:  			//   Commanded Air Fuel (Equivalence) Ratio, ((A*256)+B)/4)
										engine_data =  ((message.data[3]*256) + message.data[4])/32768;
										sprintf(buffer,"%d",(int) engine_data);
									break;
